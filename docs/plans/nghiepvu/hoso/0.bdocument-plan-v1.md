...
- **Tạo mới Hồ sơ (Create):**
    ...
    - Hệ thống tự động hiển thị các Thành phần thủ tục (Component) tương ứng với Procedure đã chọn:
        - **Đối với Component loại `Form` (đặc biệt là form "Tờ khai"):** Hiển thị động các trường nhập liệu dựa trên `FormDefinition` của Component. Dữ liệu nhập được lưu dưới dạng cấu trúc (ví dụ: JSON) vào `BDocumentData.InputData`.
    ...
    - Hệ thống tự động gán Mã hồ sơ (`Code`, có thể cấu hình quy tắc sinh mã).
    - **Trạng thái ban đầu (`WorkflowStatusId`) của hồ sơ là `null`, sẽ được cập nhật bởi hệ thống Workflow sau khi tạo.**
...
- **Tạo file từ Tờ khai (Post-Create Action):**
    - Cần có một chức năng (nút bấm, action workflow, job...) để:
        - Đọc dữ liệu JSON từ `BDocumentData.InputData` của component "Tờ khai".
...
- **Xem danh sách (Read List):**
    ...
    - Các cột cơ bản: STT, Mã hồ sơ (`Code`), Tên chủ hồ sơ (`ApplicantName`), Tên thủ tục (`Procedure.Name`), **Tên Trạng thái (lấy từ `WorkflowStatus`)**, Ngày nộp (`SubmissionDate`)/Ngày tạo (`CreationTime`).
    - Hỗ trợ tìm kiếm theo Mã hồ sơ (`Code`), Tên chủ hồ sơ (`ApplicantName`).
    - **Hỗ trợ lọc theo Trạng thái hồ sơ (sử dụng `WorkflowStatus` lookup)**, Ngày nộp (`SubmissionDate`, khoảng thời gian).
...
- **Xem chi tiết (Read Detail):**
    - Hiển thị đầy đủ thông tin của một Hồ sơ, bao gồm:
        - Thông tin Chủ hồ sơ và thông tin bổ sung (`ScopeOfActivity`, `ReceiveByPost`).
        ...
        - **Hiển thị dữ liệu các Thành phần hồ sơ:**
            - **Component "Tờ khai":** Có thể hiển thị các trường đã nhập (từ `InputData`) hoặc hiển thị link tải file đã tạo từ tờ khai (nếu `FileId` đã có).
            - **Các Component `File`:** Hiển thị tên file (lấy từ `FileManagement`) và link tải về.
- **Sửa (Update):**
    - Cho phép chỉnh sửa thông tin Hồ sơ (`ApplicantName`, `ApplicantAddress`, `ScopeOfActivity`, `ReceiveByPost`...) khi Hồ sơ ở **trạng thái (`WorkflowStatus`)** cho phép.
    - **Việc sửa dữ liệu Form "Tờ khai" (`InputData`) hoặc cập nhật/xóa/thêm mới tệp đính kèm cần có cơ chế riêng (ví dụ: trong trang chi tiết, không phải trong modal edit đơn giản).**
    - Mã hồ sơ (`Code`) không được phép sửa.
- **Xóa (Delete):**
...
- **BDocument (Entity chính):**
    ...
    - `Code` (string, unique, có thể tự sinh)
    - `ApplicantName` (string, bắt buộc)
    - `ApplicantIdentityNumber` (string?)
    - `ApplicantAddress` (string?)
    - `ApplicantEmail` (string?)
    - `ApplicantPhoneNumber` (string?)
    - `ScopeOfActivity` (string?, kiểu dữ liệu lớn)
    - `ReceiveByPost` (bool)
    - `WorkflowStatusId` (Guid?, FK - Tham chiếu đến `WorkflowStatus`, nullable)
    - `SubmissionDate` (DateTime?)
    - `ReceptionDate` (DateTime?)
    - `AppointmentDate` (DateTime?)
    - `ResultDate` (DateTime?)
    - `RejectionOrAdditionReason` (string?)
...
- **BDocumentData (Lưu dữ liệu/tham chiếu theo Component):**
    ...
    - `InputData` (string? - Lưu dữ liệu JSON cho Component loại `Form`, bao gồm cả "Tờ khai")
...
- **Màn hình Tạo mới:**
    ...
    - Phần nhập thông tin bổ sung (`ScopeOfActivity`, Checkbox `ReceiveByPost`).
...
- **Màn hình Sửa:** Chỉ sửa thông tin chính (Chủ hồ sơ, `ScopeOfActivity`, `ReceiveByPost`). Việc sửa component data cần UI khác.
- **Màn hình Xem chi tiết:** Hiển thị tất cả thông tin, bao gồm dữ liệu Tờ khai (`InputData`, dạng đọc) và link tải các file đính kèm.
...
- `Code` cần có quy tắc sinh mã và đảm bảo tính duy nhất.
- **Dữ liệu nhập vào Form "Tờ khai" (`InputData`) phải được validate dựa trên `FormDefinition`.**
- Tệp tải lên phải đáp ứng yêu cầu của `ProcedureComponent`.
- Việc thay đổi trạng thái (`WorkflowStatusId`) được quản lý bởi Workflow.
- Chỉ có thể Sửa/Xóa `BDocument` ở những trạng thái cho phép.
- **Cần có quy tắc/template để tạo file từ dữ liệu Form "Tờ khai" (`InputData`).**
...
- [x] **Bước 0: Kế hoạch tổng thể** - File này (Đã cập nhật tên định danh)
