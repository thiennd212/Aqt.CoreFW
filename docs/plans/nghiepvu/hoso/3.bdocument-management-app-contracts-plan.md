...
-   **Tệp 1:** Tạo file `BDocumentDataDto.cs` (Đại diện dữ liệu/file của một Component trong Hồ sơ)
    ```csharp
    using System;
    ...
    /// <summary>
    /// Đại diện cho dữ liệu hoặc file liên kết với một ProcedureComponent cụ thể
...
-   **Tệp 3:** Tạo file `BDocumentListDto.cs` (DTO cho danh sách Hồ sơ - Cập nhật)
    ...
    /// <summary>
    /// DTO rút gọn để liệt kê BDocuments.
    /// </summary>
    public class BDocumentListDto : EntityDto<Guid>
    {
        ...
        public DateTime CreationTime { get; set; } // Ngày tạo (hoặc SubmissionDate nếu dùng)
    }
    ```
...
-   **Tệp 4:** Tạo file `CreateBDocumentInputDto.cs` (Input để tạo Hồ sơ - Cập nhật)
    ...
    public class CreateBDocumentInputDto
    {
        ...
        // Bao gồm cả dữ liệu JSON của "Tờ khai" trong InputData của item tương ứng
        [Required]
        public List<CreateBDocumentComponentDataInputDto> ComponentData { get; set; } = new List<CreateBDocumentComponentDataInputDto>();

        // Code tự sinh, WorkflowStatusId ban đầu null
    }
    ```

-   **Tệp 5:** Tạo file `CreateBDocumentComponentDataInputDto.cs` (Input cho dữ liệu component khi tạo)
    ```csharp
    using System;
    using System.ComponentModel.DataAnnotations;
    ...
    /// <summary>
    /// Đại diện cho dữ liệu đầu vào cho một thành phần duy nhất trong quá trình tạo BDocument.
    /// </summary>
...
-   **Tệp 7:** Tạo file `AddOrUpdateBDocumentDataInputDto.cs` (Input để thêm/sửa dữ liệu Component sau khi tạo)
    ```csharp
    using System;
    using System.ComponentModel.DataAnnotations;
    ...
    ```
-   **Tệp 8:** Tạo file `ChangeBDocumentStatusInputDto.cs` (Input để đổi trạng thái)
    ```csharp
    using System;
    using System.ComponentModel.DataAnnotations;
    using Aqt.CoreFW.BDocuments; // Constants
...
-   **Tệp 9:** Tạo file `GetBDocumentsInput.cs` (Input cho GetList)
    ```csharp
    using System;
    using Volo.Abp.Application.Dtos;
    ...
    public class GetBDocumentsInput : PagedAndSortedResultRequestDto
    {
        ...
        public Guid? WorkflowStatusId { get; set; } // Lọc theo Trạng thái
