...
*   **Loại bỏ** các thuộc tính Tờ khai, **thêm** `PhamViHoatDong`, `DangKyNhanQuaBuuDien`.
*   **Trạng thái nullable:** `TrangThaiHoSoId` là `Guid?`.
...
*   Chứa `DuLieuNhap` (JSON Tờ khai) hoặc `FileId`.
...
*   Đảm bảo tính nhất quán (`MaHoSo` unique, `ProcedureId` hợp lệ).
...
-   **Nội dung:**
    ...
    public class BDocumentManager : DomainService
    {
        ...
        private async Task ValidateMaHoSoUniquenessAsync([NotNull] string maHoSo, Guid? excludeId = null)
        {
            ...
            {
                throw new UserFriendlyException(L[CoreFWDomainErrorCodes.BDocumentCodeAlreadyExists, maHoSo]);
            }
        }
