        ...
        : IMappingAction<BDocument, BDocumentExcelDto>, ITransientDependency
    {
        ...
        public async Task Process(BDocument source, BDocumentExcelDto destination, ResolutionContext context)
        {
            ...
            if (source.WorkflowStatusId.HasValue)
            {
                 ...
                 destination.WorkflowStatusName = status?.Name ?? string.Empty;
            }
            else
            {
                 destination.WorkflowStatusName = ""; // Or a localized "Not Set" string
            }
         ...
         private async Task ProcessAsync(BDocument source, BDocumentExcelDto destination, ResolutionContext context)
        {
            ...
            destination.ProcedureName = procedure?.Name ?? string.Empty;
             if (source.WorkflowStatusId.HasValue) {
                 var status = await _statusRepository.FindAsync(source.WorkflowStatusId.Value);
                 destination.WorkflowStatusName = status?.Name ?? string.Empty;
             } else { destination.WorkflowStatusName = ""; }
            destination.ScopeOfActivity = source.ScopeOfActivity;
            destination.ReceiveByPost = source.ReceiveByPost ? "Yes" : "No";
            destination.SubmissionDate = source.SubmissionDate;
            destination.ReceptionDate = source.ReceptionDate;
            destination.AppointmentDate = source.AppointmentDate;
            destination.ResultDate = source.ResultDate;
    ...
    public class BDocumentApplicationAutoMapperProfile : Profile
    {
        public BDocumentApplicationAutoMapperProfile()
        {
            ...
            CreateMap<BDocument, BDocumentDto>()
                 ...
                 .ForMember(dest => dest.WorkflowStatus, opt => opt.Ignore()); // Enrich later

            CreateMap<BDocument, BDocumentListDto>()
                 .ForMember(dest => dest.ProcedureName, opt => opt.Ignore()) // Enrich later
                 .ForMember(dest => dest.WorkflowStatusName, opt => opt.Ignore()) // Enrich later
                 .ForMember(dest => dest.WorkflowStatusColorCode, opt => opt.Ignore()); // Enrich later
            ...
            CreateMap<BDocument, BDocumentExcelDto>()
                 ...
                 .ForMember(dest => dest.WorkflowStatusName, opt => opt.Ignore())   // Handle in AppService or Action
                 .ForMember(dest => dest.ReceiveByPost, opt => opt.MapFrom(src => src.ReceiveByPost ? "Yes" : "No")); // Simple bool mapping
...
-   **Nội dung:**
    ...
    public class BDocumentAppService : ApplicationService, IBDocumentAppService
    {
        ...
        public async Task<BDocumentDto> CreateAsync(CreateBDocumentInputDto input)
        {
            ...
            string code = "HS-" + Clock.Now.ToString("yyyyMMddHHmmssfff");
            ...
            if (input.ComponentData != null && input.ComponentData.Any())
            {
                ...
                foreach (var componentInput in input.ComponentData)
                {
                    ...
                    _bDocumentManager.ValidateComponentDataType(componentDef, componentInput.InputData, componentInput.FileId);

                    // Validate declaration form data (if applicable)
                    if (componentDef.Code == BDocumentConsts.DeclarationFormComponentCode)
                    {
                       // ValidateDeclarationFormData(componentInput.InputData, componentDef.FormDefinition);
                    }
                    ...
                    await _bDocumentManager.AddOrUpdateComponentDataAsync(
                        ...
                        componentInput.InputData, // Contains declaration JSON
        ...
        public async Task<BDocumentDto> GetAsync(Guid id)
        {
            ...
            if (bDocument.WorkflowStatusId.HasValue)
            {
                 var status = await _statusRepository.FindAsync(bDocument.WorkflowStatusId.Value);
                 if (status != null)
                 {
                     dto.WorkflowStatus = ObjectMapper.Map<WorkflowStatus, WorkflowStatusDto>(status);
                 }
            }
                ...
                if (fileIds.Any())
                {
                    ...
                    try
                    {
                        ...
                        foreach(var fileId in fileIds)
                        {
                           ...
                           var fileDtoFm = await _fileAppService.GetAsync(fileId); // File DTO from FileManagement
                           ...
                           fileInfos[fileId] = ObjectMapper.Map<EasyAbp.FileManagement.Files.Dtos.FileInfoDto, FileInfoDto>(fileDtoFm); // Explicit map
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public async Task<PagedResultDto<BDocumentListDto>> GetListAsync(GetBDocumentsInput input)
        {
            var totalCount = await _bDocumentRepository.GetCountAsync(
                ...
                // Add filter for ReceiveByPost if implemented
            );
            ...
            if (dtos.Any())
            {
                ...
                var statusIds = dtos.Where(d => d.WorkflowStatusId.HasValue).Select(d => d.WorkflowStatusId!.Value).Distinct().ToList();
                ...
                foreach (var dto in dtos)
                {
                    if (procedures.TryGetValue(dto.ProcedureId, out var procedure))
                    {
                     ...
                     if (dto.WorkflowStatusId.HasValue && statuses.TryGetValue(dto.WorkflowStatusId.Value, out var status))
                    {
                        dto.WorkflowStatusName = status.Name;
                        dto.WorkflowStatusColorCode = status.ColorCode;
                    }
                    else
                    {
                        // Handle null status name if needed (e.g., localized "Not Set")
                        dto.WorkflowStatusName = _localizer["NotSet"] ?? "Not Set";
                    }
                }
            }
        ...
        public async Task<BDocumentDataDto> AddOrUpdateDataAsync(Guid id, AddOrUpdateBDocumentDataInputDto input)
        {
            ...
            //    ValidateDeclarationFormData(input.InputData, component.FormDefinition);
            // }
        ...
        public async Task<FileInfoDto> GenerateDeclarationFileAsync(Guid id)
        {
            ...
            try {
                ...
                throw new UserFriendlyException(L[CoreFWDomainErrorCodes.InvalidInputDataForDeclaration, ex.Message]);
            }
            ...
            string generatedFileName = $"ToKhai_{bDocument.Code}.pdf"; // Example name
            ...
            try {
                 ...
                 generatedFileBytes = System.Text.Encoding.UTF8.GetBytes($"Simulated PDF content for {bDocument.Code}\nData: {declarationData.InputData}"); // Simulation
            } catch (Exception ex) {
            ...
            FileInfoDto createdFileDto; // Use FileInfoDto from Contracts
            try {
                var createFileInput = new CreateFileInput
                {
                    FileContainerName = BDocumentConsts.FileContainerName, // Get from Consts
                    FileName = generatedFileName,
                    Bytes = generatedFileBytes,
                    FileType = FileType.RegularFile
                };
                var fmFileDto = await _fileAppService.CreateAsync(createFileInput); // DTO from FileManagement
                createdFileDto = ObjectMapper.Map<EasyAbp.FileManagement.Files.Dtos.FileInfoDto, FileInfoDto>(fmFileDto); // Map to our DTO
            } catch (Exception ex) {
            ...
            await _bDocumentManager.AddOrUpdateComponentDataAsync(bDocument, declarationData.ProcedureComponentId, declarationData.InputData, createdFileDto.Id);
            // Optionally clear InputData after successful generation:
        ...
        public async Task<IRemoteStreamContent> GetListAsExcelAsync(GetBDocumentsInput input)
        {
            ...
            var bDocuments = await _bDocumentRepository.GetListAsync(
             ...
             if (bDocuments.Any())
             {
                 ...
                 var statusIds = bDocuments.Where(d=>d.WorkflowStatusId.HasValue).Select(d => d.WorkflowStatusId!.Value).Distinct().ToList();
                 var procedures = (await _procedureRepository.GetListAsync(ids: procedureIds)).ToDictionary(p => p.Id);
                 var statuses = new Dictionary<Guid, WorkflowStatus>();
                 ...
                 {
                     var bDoc = bDocuments.First(b => b.Code == dto.Code); // Find original by unique key
                     if (procedures.TryGetValue(bDoc.ProcedureId, out var p)) dto.ProcedureName = p.Name;
                     if (bDoc.WorkflowStatusId.HasValue && statuses.TryGetValue(bDoc.WorkflowStatusId.Value, out var s)) dto.WorkflowStatusName = s.Name;
                     else dto.WorkflowStatusName = ""; // Handle null status
        ...
        // --- Helper Methods ---
         ...
         // private void ValidateDeclarationFormData(string? inputData, string? formDefinitionJson) {
         //    if (string.IsNullOrWhiteSpace(inputData)) throw new UserFriendlyException("Declaration form data cannot be empty.");
...
-   **Nội dung cần thêm/cập nhật:**
    ```csharp
     [DependsOn(
         ...
         typeof(FileManagementApplicationModule), // Maybe needed if using IFileManager directly
     ...
     public class CoreFWApplicationModule : AbpModule
     {
         public override void ConfigureServices(ServiceConfigurationContext context)
         {
              Configure<AbpAutoMapperOptions>(options =>
              {
                  ...
                  options.AddProfile<BDocumentApplicationAutoMapperProfile>(); // Explicitly add profile
              });
...
*   **Validation:** Validation dữ liệu Tờ khai động (`InputData`) cần được xem xét kỹ lưỡng.