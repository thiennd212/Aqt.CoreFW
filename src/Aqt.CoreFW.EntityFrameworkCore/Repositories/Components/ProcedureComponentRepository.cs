    ...
    IProcedureComponentRepository
{
    ...
    /// <summary>
    /// Check if a ProcedureComponent is linked to a specific Procedure
    /// </summary>
    /// <param name="componentId"></param>
    /// <param name="procedureId"></param>
    /// <returns></returns>
    public async Task<bool> IsLinkedToProcedureAsync(Guid componentId, Guid procedureId)
    {
        var dbContext = await GetDbContextAsync();
        var exists = await dbContext.ProcedureComponentLinks
            .AnyAsync(l => l.ProcedureComponentId == componentId && l.ProcedureId == procedureId);
        return exists;
    }

    /// <summary>
    /// Get a list of ProcedureComponents by their IDs.
    /// </summary>
    ...
    public async Task<List<ProcedureComponent>> GetListByIdsAsync(List<Guid> ids, bool includeDetails = false, CancellationToken cancellationToken = default)
    {
        var query = await GetQueryableWithDetailsAsync(includeDetails);
    ...
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    public async Task<List<ProcedureComponent>> GetListByProcedureAsync(Guid procedureId, ComponentType? type = null, bool includeDetails = false, CancellationToken cancellationToken = default)
    {
        var query = await GetQueryableWithDetailsAsync(includeDetails);
        return await query.AsNoTracking()
            .Where(p => p.ProcedureLinks.Any(l => l.ProcedureId == procedureId) && (!type.HasValue || p.Type == type.Value))
            .ToListAsync(GetCancellationToken(cancellationToken));
    }
}